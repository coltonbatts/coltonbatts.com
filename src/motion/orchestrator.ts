...
class RiveOrchestrator {
		...
		this.sectionObserver = new IntersectionObserver(
			(entries) => {
				entries.forEach((entry) => {
					...
					this.emitToPlayer(
						id,
						nowVisible ? 'section-enter' : 'section-exit',
						{
							active: nowVisible,
	...
	private bindGlobalSignals() {
		...
		window.addEventListener('keydown', (event) => {
			if (!(event.shiftKey && event.key.toLowerCase() === 'm')) return;
			this.toggleHud();
		});
	...
	private startDevSummary() {
		...
		this.devSummaryTimer = window.setInterval(() => {
			...
			console.info(
				`[MotionOS] players=${count} visible=${visible} reduced=${this.reducedMotion} heartbeat=${this.heartbeatFps.toFixed(0)}fps-ish idle=${this.isIdle}`
			);
		}, 10000);
	}
}
